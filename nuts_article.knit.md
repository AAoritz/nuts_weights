...
<!-- ### Converting Between Classifications Using Spatial Interpolation -->
...
We illustrate this conversion with an example in @fig-absolute-example where two source regions, CA111 and CA112, contain population and a variable of interest measured in absolute terms. Region CA111 has a population of 500,000 and a value of 85,000 for the variable, while region CA112 has a population of 300,000 and a value of 50,000. When the NUTS classification is revised, these boundaries change such that the territory is reorganized into two new regions: CA113 and CA114.

The conversion process begins by calculating how the population of each source region is distributed across the new regional boundaries. Using population as the covariate, we determine that 390,000 inhabitants of CA111 fall within the new CA113 boundary, while 110,000 inhabitants of CA111 fall within CA114. The entirety of CA112's population (300,000) is incorporated into CA114.

To convert the variable values, we apply proportional allocation based on these population distributions. For the portion of CA111 that becomes CA113, the converted value is calculated as 85,000 × (390,000 / 500,000) = 66,300. Similarly, the portion of CA111 that contributes to CA114 yields 85,000 × (110,000 / 500,000) = 18,700. The contribution from CA112 to CA114 is straightforward, as the entire region is absorbed: 50,000 × (300,000 / 300,000) = 50,000.

The final step involves aggregating contributions to each target region. Region CA113 receives only one contribution (from CA111), resulting in a final value of 66,300. Region CA114, however, receives contributions from both source regions, and these are summed to produce the final value: 18,700 + 50,000 = 68,700. The resulting dataset shows CA113 with a population of 390,000 and a variable value of 66,300, while CA114 has a population of 410,000 and a variable value of 68,700.
...
@fig-relative-example uses the same territorial reorganization as in the previous section, but the variable of interest is measured as a percentage. Region CA111, with a population of 500,000, has a value of 8%, while region CA112, with a population of 300,000, has a value of 11%. When converting these values to the new NUTS classification (regions CA113 and CA114), the method preserves the original percentages for areas originating from a single source region and calculates weighted averages for areas receiving contributions from multiple sources.

For the portion of CA111 that becomes CA113, the percentage value remains unchanged at 8%, as this area derives entirely from one source region. The calculation is straightforward: (8 × 390,000) / 390,000 = 8.0%. For region CA114, however, the situation is more complex, as this region incorporates territory from both CA111 and CA112, each with different percentage values.

The appropriate method for CA114 involves calculating a population-weighted average of the contributing percentages. The portion of CA111 contributing to CA114 has a population of 110,000 with a value of 8%, while the portion from CA112 has a population of 300,000 with a value of 11%. The weighted average is computed as [(8 × 110,000) + (11 × 300,000)] / 410,000. This calculation yields (880,000 + 3,300,000) / 410,000 = 10.2%.

The resulting dataset shows CA113 with a population of 390,000 and a value of 8.0%, reflecting its origin entirely from CA111. Region CA114, with a population of 410,000, has a value of 10.2%, representing the weighted combination of its constituent parts from both source regions. This weighted averaging approach ensures that the relative values in the converted dataset accurately reflect the demographic composition of each new region.
...
In our first validation exercise in @fig-validation-eurostat, we conduct a round-trip conversion stress test using land area and 2021 population derived directly from the crosswalk tables, covering 1,209 NUTS 3 regions for land area and 1,208 for population. Starting from NUTS 2024 values, we apply the full sequence 2024→2021→2016→2013→2010→2006→2010→2013→2016→2021→2024 of ten conversion steps using each of the six interpolation weights independently, then measure the absolute relative error between the recovered NUTS 2024 values and the originals. Because the same weights are applied forward and backward over the same crosswalk, this design primarily tests the internal consistency and invertibility of the conversion weights, not their accuracy against an external ground truth; the very low MAPEs reported below should be read accordingly. The Eurostat comparison in the next section, which checks converted values against independently published statistics, is the primary test of accuracy. @fig-validation-eurostat shows the distribution of errors for all NUTS 3 regions on a logarithmic scale, with diamonds marking the mean absolute percentage error (MAPE) computed over all regions for each weight and variable combination. Because most NUTS 3 regions were never subject to boundary changes across the five version transitions — 981 of 1,209 (land area) and 948 of 1,208 (population) return from the round-trip with negligible error regardless of weight — overall MAPEs are low and primarily reflect interpolation error concentrated in the minority of regions that genuinely changed. For land area, the area weight achieves the lowest error overall (MAPE 0.18%), followed by the artificial surface weights (2.5%), built-up volume (4.1%), and both population weights (4.4%); the mirror pattern holds for population, where the Population 2021 weight achieves the lowest MAPE (0.08%) and the area weight performs worst (3.0%). These results confirm that each variable is best preserved by its own matching weight and that weight selection has a material effect for regions undergoing boundary reforms.

![**Round-trip conversion error for land area and population at NUTS 3 level.** Errors are measured across a ten-step conversion chain (2024→2021→2016→2013→2010→2006→2010→2013→2016→2021→2024), covering all 1,209 (land area) and 1,208 (population) NUTS 3 regions in the crosswalk. Each dot represents one NUTS 3 region with error above 0.001%; diamonds mark the mean absolute percentage error (MAPE) per weight and variable, computed over all regions including unchanged ones. `{\footnotesize\itshape Source: NUTS conversion tables (\texttt{areaKm}, \texttt{pop21}).}`{=latex}](figs/validation_eurostat.png){#fig-validation-eurostat}
...
In our second validation exercise in @fig-validation-gdp-pop, we validate the accuracy of the conversion weights against independently published Eurostat data. Eurostat releases regional statistics under both the outgoing and incoming NUTS versions during transition periods, allowing a direct comparison of converted values against the ground truth. We convert NUTS 2021 regional statistics to NUTS 2024 using each of the six interpolation weights and compare the results against NUTS 2024 values directly reported by Eurostat. The comparison is restricted to the 20 NUTS 3 regions that underwent genuine boundary changes (splits or mergers) between NUTS 2021 and 2024, evaluated separately for population (reference year: 2023) and GDP at current market prices (most recent year: 2021–2022).

@fig-validation-gdp-pop shows the distribution of absolute relative errors by weight on a logarithmic scale. For population, the 2021 population weight achieves the best accuracy (MAPE 0.4%), followed by the 2011 population weight (0.9%), built-up volume (2.5%), both artificial surface weights (4.8%), and the area weight performing worst (MAPE 13.1%). For GDP, population-based weights again perform best (MAPE 8.1% and 8.2% for the 2021 and 2011 vintages, respectively), followed by built-up volume (10.6%) and the artificial surface weights (14.1% and 14.3%), with the area weight yielding the largest errors (MAPE 24.3%). The log scale reveals that errors span several orders of magnitude within each weight, from near-zero to tens of percent, with population-based weights consistently shifted toward the lower end of the range. GDP errors are systematically higher than population errors — roughly 20 percentage points across all weights — reflecting the more spatially concentrated distribution of economic activity. Even the best-performing weight leaves a GDP MAPE of around 8%, a material error for a headline economic variable, and users requiring high accuracy on economic indicators for the affected regions should treat converted GDP figures with corresponding caution. This comparison is based on a small evaluation set — 20 NUTS 3 regions and two variables (population, GDP) — so the relative ranking of weights is informative but should not be over-generalised to other socioeconomic variables or to country compositions different from the 2021-to-2024 transition. Within this scope, population-based weights perform best for both population and GDP, while weight selection has a material effect on conversion accuracy.

![**Validation of conversion weights against independently published Eurostat data for the NUTS 2021-to-2024 transition.** Boxplots show the distribution of absolute relative errors, on a logarithmic scale, when NUTS 2021 regional statistics are converted to NUTS 2024 and compared against Eurostat's directly reported NUTS 2024 values. Left panel: population (20 changed NUTS 3 regions, reference year 2023). Right panel: GDP at current market prices (20 changed NUTS 3 regions, most recent year 2021–2022). MAPE values are reported at the right edge of each panel. `{\footnotesize\itshape Source: Eurostat NUTS regional statistics (population and GDP at current market prices) and conversion tables.}`{=latex}](figs/validation_gdp_pop_singleyear.png){#fig-validation-gdp-pop}
...
The authors would like to thank European Commission's DG REGIO for supporting and funding the development of this work. Moritz Hennicke acknowledges funding under Marie Sklowdoska Curie Actions with grant agreement ID: 101211177.
