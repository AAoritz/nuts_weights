...
![**Flowchart of conversion table construction.** `{\footnotesize\emph{Note:} The workflow proceeds in two stages: a geometric intersection of administrative boundaries, followed by a raster overlay of 100m population, artificial-surface, and built-up-volume grids to derive the interpolation weights.}`{=latex}](figs/workflow.png){#fig-workflow width=50%}
...
The conversion tables can be downloaded via... [LINK]. The tables are divided into `{r} nr_files` csv files, where each file contains flows of area, population, artificial surfaces and built-up volume at the level of a combination of NUTS codes in two different versions (either for NUTS levels 1, 2 or 3). Every csv file contains `{r} nr_cols` columns with the variables described in @tbl-variables.
...
![**Round-trip conversion error for land area and population at NUTS 3 level.** Errors are measured across a ten-step conversion chain (2024→2021→2016→2013→2010→2006→2010→2013→2016→2021→2024), covering all 1,209 (land area) and 1,208 (population) NUTS 3 regions in the crosswalk. Each dot represents one NUTS 3 region with error above 0.001%; diamonds mark the mean absolute percentage error (MAPE) per weight and variable, computed over all regions including unchanged ones. `{\footnotesize\itshape Source: NUTS conversion weights crosswalk (\texttt{areaKm}, \texttt{pop21}).}`{=latex}](figs/validation_eurostat.png){#fig-validation-eurostat}
...
![**Validation of conversion weights against independently published Eurostat data for the NUTS 2021-to-2024 transition.** Boxplots show the distribution of absolute relative errors, on a logarithmic scale, when NUTS 2021 regional statistics are converted to NUTS 2024 and compared against Eurostat's directly reported NUTS 2024 values. Left panel: population (20 changed NUTS 3 regions, reference year 2023). Right panel: GDP at current market prices (20 changed NUTS 3 regions, most recent year 2021–2022). MAPE values are reported at the right edge of each panel. `{\footnotesize\itshape Source: Eurostat NUTS regional statistics (population and GDP at current market prices).}`{=latex}](figs/validation_gdp_pop_singleyear.png){#fig-validation-gdp-pop}
...
<!-- As shown in @fig-conversion-pl, the choice of NUTS classification version can visibly affect the apparent spatial pattern of regional economic growth: converting the same Eurostat GDP per capita data to NUTS 2010 and NUTS 2013 classifications yields different maps of inequality dynamics in Poland, illustrating why version-consistent data are important for longitudinal regional analysis. -->

<!-- ![Example: Regional economic growth in Poland in NUTS classifications 2010 and 2013](figs/poland_nuts_gdp_comparison.png){#fig-conversion-pl} -->


## Code availability {.unnumbered}

<!-- - The code used for the construction of the transition tables can be accessed: [https://....](UPLOAD TO GITLAB/GITHUB BEFORE SUBMISSION) -->
- Code for figures and tables presented in this article can be found in the Github repository: [UPLOAD TO GITLAB/GITHUB BEFORE SUBMISSION](https://....)
